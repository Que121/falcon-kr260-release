...
#define IC 512
#define NPT 128
void scr_head16(const i16*, const i16*, i16*, const int*, const int*, const int*, int);
static short wbuf[3*IC*IC], ibuf[IC*NPT], obuf_s[IC*NPT];
...
int main(){
  ...
  printf("shifts %d %d %d, sout %.3e\n", shifts[0],shifts[1],shifts[2], sout);
  scr_head16(wts, input, out, mult, bias, shifts, 3);
  double dot=0,na=0,nb=0,maxd=0;
  for(int i=0;i<IC*NPT;i++){ float sim=(float)((int)out[i])*sout; dot+=sim*ref[i]; na+=(double)sim*sim; nb+=(double)ref[i]*ref[i]; double d=fabs(sim-ref[i]); if(d>maxd)maxd=d; }
  printf("HLS csim cosine vs FP32 = %.5f, maxd=%.4f\n", dot/(sqrt(na)*sqrt(nb)+1e-9), maxd);
  return 0;
}

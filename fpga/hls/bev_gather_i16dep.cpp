#include "bev_gather_i16dep.hpp"

// INT16-vt deploy gather: same segmented scatter-add + WCET as bev_gather_deploy.cpp (all per-frame
// tensors AND the static index tables over m_axi to DDR), only the requantize/write widens to INT16.
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=64 avg=14
            ...
            const feat_wide_t fv = feat[rank_feat[i]];
        MAC:
            for (int c = 0; c < C; c++) {
#pragma HLS UNROLL

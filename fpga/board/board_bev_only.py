#!/usr/bin/env python3
"""BEV-only batched: FP32 vt_out (from dumped frames) -> BEV xmodel (3 DPU + 2 resize-IP) -> predicter
-> occ argmax, over N frames. Isolates the BEV-stage INT8 quality (no image/gather confound) for mIoU.
  python3 board_bev_only.py <frames_dir> <N> <out_occ.npy> <bev_xmodel> [predicter.npz]
"""
...
def main():
    ...
    M = len(frames); print("BEV-only %d frames | %s" % (M, os.path.basename(BEV_XM)), flush=True)
    ...
    # one big chunk (all M) but stream per-stage; load DPU once per DPU subgraph (interleaved with resize)
    st = [dict() for _ in range(M)]
    for k in range(M):
        vt = np.load(frames[k])["vt_out"].astype(np.float32)[None].transpose(0, 2, 3, 1)
    ...
    for si, sg in enumerate(order):
        if dev(sg) == "DPU":
            ...
            if Ho > Hin:
                bit = RZ25 if Ho == 100 else RZ100
                if cur != bit: cur = bit
                Wo = int(ot.dims[2]); fpo = t_fp(ot) if t_fp(ot) is not None else None
                for k in range(M):
                    x, fpx = st[k][it.name]
                    st[k][ot.name] = (resize(bit, x, fpx, fpo if fpo is not None else fpx, (Ho, Wo)), fpo if fpo is not None else fpx)
            else:
                # same-size CPU op: Hardtanh-clamp == requant to ot's fix_point WITH saturation (ARM, trivial)
                fpo = t_fp(ot)
                for k in range(M):
    ...
    if dov is not None: del dov
    ot = list(order[-1].get_output_tensors())[0]
    for k in range(M):
        arr, fp = st[k][ot.name]
        ...
        out[k] = (x @ W2.T + b2).reshape(200, 200, 16, 18).argmax(-1).astype(np.uint8)

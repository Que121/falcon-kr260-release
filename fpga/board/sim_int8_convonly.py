...
q = torch_quantizer("test", make(), (dummy,), output_dir="/work/quantize_result_clamp")
qm = q.quant_model
x = torch.from_numpy(np.load("/work/bev_test_input.npy").astype("float32"))
with torch.no_grad():
    y = qm(x).numpy()
np.save("/work/bev_sim_int8_convonly.npy", y)
print("sim int8 convonly", y.shape, float(y.min()), float(y.max()))
